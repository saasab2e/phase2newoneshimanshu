...
const TENANT_BEHAVE_PERMISSIONS = [
  'view_team_activity',
  'reports_read',
  'view_activity_log',
  'manage_settings',
...
export default function TenantBehaveLayout({ children }: { children: React.ReactNode }) {
  const avatarUrl =
    ...
    <PasswordResetGuard>
      <div className="min-h-screen bg-slate-50 font-['Arimo',sans-serif]">
        <Sidenav avatarUrl={avatarUrl}>
          <PermissionRouteGuard anyPermissions={TENANT_BEHAVE_PERMISSIONS}>{children}</PermissionRouteGuard>
        </Sidenav>
      </div>
    </PasswordResetGuard>
  );
