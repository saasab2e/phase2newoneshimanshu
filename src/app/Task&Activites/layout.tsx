...
export default function TaskActivitiesLayout({ children }: { children: React.ReactNode }) {
  const avatarUrl =
    ...
    <PasswordResetGuard>
      <div className="min-h-screen bg-slate-50 font-['Arimo',sans-serif]">
        <Sidenav avatarUrl={avatarUrl}>
          <PermissionRouteGuard anyPermissions={MODULE_ACCESS_MAP.Tasks}>{children}</PermissionRouteGuard>
        </Sidenav>
      </div>
    </PasswordResetGuard>
  );
