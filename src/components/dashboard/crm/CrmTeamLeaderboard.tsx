...
import { formatNum, relativeTime, useCrmDashboard } from './crmShared';

type Props = { overview: CrmOverview | null; loading?: boolean; className?: string };
type Quick = 'top' | 'overdue' | 'all';
...
export function CrmTeamLeaderboard({ overview, className = '' }: Props) {
  const { filters, setFilters, bumpRefresh, openDrillDown } = useCrmDashboard();
  ...
  const filtered = useMemo(() => {
    ...
    if (quick === 'overdue') list = list.filter((r) => Number(r.overdueFollowups || 0) > 0);
    list = [...list].sort(
      (a, b) => (b.conversions || 0) - (a.conversions || 0) || (b.assignedLeads || 0) - (a.assignedLeads || 0),
    );
    if (needle || quick === 'overdue') return list.slice(0, 8);
  ...
  const openDetails = (r: (typeof all)[number]) => {
    openDrillDown({
      title: r.name,
      href: '/team',
      rows: [
  ...
  return (
    <section
      className={`relative flex h-full min-h-[240px] flex-col rounded-[1.25rem] border border-slate-100/80 bg-white p-3.5 shadow-[0_14px_40px_-28px_rgba(15,23,42,0.22)] ${className}`}
    >
            ...
            {([
              ...
              <button
                ...
                className={`rounded-full px-2 py-1 text-[11px] font-semibold ${
                  quick === id ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                {label}
              </button>
      ...
      <ul className="min-h-0 flex-1 space-y-1.5 overflow-y-auto pr-0.5">
        {filtered.length ? (
              ...
              <li key={r.id}>
                <div
                  className={`flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 transition ${
                    on ? 'bg-slate-900 text-white' : 'bg-slate-50/80 hover:bg-slate-100'
                  }`}
                >
                  <button type="button" onClick={() => applyPerson(r.id)} className="flex min-w-0 flex-1 items-center gap-2.5 text-left">
                    <span
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold ${
                        on ? 'bg-white/15 text-white' : 'bg-[#334155] text-white'
                      }`}
                    >
                      {initials(r.name)}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className={`block truncate text-[13px] font-semibold ${on ? 'text-white' : 'text-slate-900'}`}>
                        {r.name}
                      </span>
                      <span className={`mt-0.5 flex flex-wrap items-center gap-1 ${on ? 'text-white/80' : 'text-slate-500'}`}>
                        <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold tabular-nums ${on ? 'bg-white/10' : 'bg-white text-slate-600 ring-1 ring-slate-100'}`}>
                          {formatNum(r.assignedLeads)} leads
                        </span>
                        <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold tabular-nums ${on ? 'bg-white/10' : 'bg-white text-slate-600 ring-1 ring-slate-100'}`}>
                          {formatNum(r.conversions)} won
                        </span>
                        {(r.overdueFollowups || 0) > 0 ? (
                          <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold tabular-nums ${on ? 'bg-rose-400/30 text-white' : 'bg-rose-50 text-rose-700 ring-1 ring-rose-100'}`}>
                            {formatNum(r.overdueFollowups)} od
                          </span>
                        ) : null}
          ...
          <li className="py-6 text-center text-[12px] text-slate-400">No teammates match</li>
        )}
      </ul>
      {filtered[0]?.lastActivity && filtered.length === 1 ? (
        <p className="mt-2 text-[10px] text-slate-400">Last activity {relativeTime(filtered[0].lastActivity)}</p>
      ) : null}
    </section>
  );
}
